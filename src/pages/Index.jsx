...
const Index = () => {
  ...
  const [startFare, setStartFare] = useState(0);
  const [distanceFare, setDistanceFare] = useState(0);
  const [additionalPassengersFare, setAdditionalPassengersFare] = useState(0);
  const [childrenFare, setChildrenFare] = useState(0);
  const [luggageFare, setLuggageFare] = useState(0);
  const [cyclesFare, setCyclesFare] = useState(0);
  const [waitingTimeFare, setWaitingTimeFare] = useState(0);
  ...
  const calculateFare = () => {
    ...
    setDistanceFare(distanceFare);
    setAdditionalPassengersFare(additionalPassengersFare);
    setChildrenFare(childrenFare);
    setLuggageFare(luggageFare);
    setCyclesFare(cyclesFare);
    setWaitingTimeFare(waitingTimeFare);
          ...
          <GridItem>
            <Box bg="white" p={6} borderRadius="md" boxShadow="md">
              ...
              <VStack align="start" spacing={4} mt={8}>
                ...
                <Text fontSize="md">Start Fare: £{startFare.toFixed(2)}</Text>
                <Text fontSize="md">Distance Fare: £{distanceFare.toFixed(2)}</Text>
                <Text fontSize="md">Additional Passengers: £{additionalPassengersFare.toFixed(2)}</Text>
                <Text fontSize="md">Children: £{childrenFare.toFixed(2)}</Text>
                <Text fontSize="md">Luggage, Prams, Dogs: £{luggageFare.toFixed(2)}</Text>
                <Text fontSize="md">Cycles: £{cyclesFare.toFixed(2)}</Text>
                <Text fontSize="md">Waiting Time: £{waitingTimeFare.toFixed(2)}</Text>
              </VStack>
            </Box>
          </GridItem>
        </Grid>
